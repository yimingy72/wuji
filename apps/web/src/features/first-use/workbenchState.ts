...
export function taskStatusLabel(
  ...
  if (launch?.phase_status === 'pending' || launch?.phase_status === 'running') return '正在启动';
  if (launch?.phase_status === 'failed') return '启动失败';
  if (launch?.phase_status === 'blocked') return '启动受阻';
  if (launch?.phase_status === 'cancelled') return '取消受理';
  if (task.desired_state === 'cancel' && task.observed_state === 'quiescing') return '正在停止';
  ...
  if (task.observed_state === 'quiescing') return '正在停止';

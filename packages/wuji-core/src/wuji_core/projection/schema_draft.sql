-- P13 UNNUMBERED DRAFT. Not installed, imported or executed by P13 core.
-- Main must allocate an upgrade after P09's shared window and bind the actual
-- application_role identifier. No schema_migration head is claimed here.
-- The migration owner, never a product GET, installs these derived read tables.
...
-- A narrow safe-field reader, not SELECT permission on Assignment/credentials.
-- P09 actual Assignment + immutable registered Run identity + start operation +
-- actual matching readable outbox event are all required. Older rows without
-- that real producer return no origin. event_seq is used only in the join and
-- is not returned even to this consumer: its sole output is the timestamp.
...
-- Integrator binds this identifier through psycopg.sql.Identifier in its assigned
-- upgrade, or uses psql's quoted identifier variable in this draft explicitly.
GRANT EXECUTE ON FUNCTION vnext.projection_reader(text,text,text,text,integer) TO :"application_role";
GRANT EXECUTE ON FUNCTION vnext.projection_run_origin(text,text,text,text) TO :"application_role";
GRANT SELECT,INSERT ON vnext.projection_materialization,vnext.projection_view,vnext.projection_cursor TO :"application_role";
-- No application UPDATE/DELETE, domain grants, new Task events, or migration run.

...
from wuji_core.scheduling.policy import Candidate, SchedulerPolicy, SchedulingSnapshot, WorkKey
from wuji_core.scheduling.triggers import TriggerRepository, block, require_admission, rows, state_row
    ...
    def acquire(self):
        ...
        with self.connection.transaction():
            ...
            acquired, pid = self.connection.execute(
                "SELECT pg_try_advisory_lock(%s,%s),pg_backend_pid()", self.LOCK,
            ).fetchone()
        if acquired:
            self._pid = pid
        return acquired
...
class WorkRepository:
    def register(self, tx, *, key, kind, priority=0):
        ...
        if (not isinstance(key, WorkKey) or kind not in {"reason", "explore", "report"}
            or type(priority) is not int or not -10 <= priority <= 10
        ...
        count = tx.connection.execute(
            "SELECT count(*) FROM vnext.work_item WHERE tenant_id=%s AND project_id=%s AND task_id=%s", tx.owner,
        ).fetchone()[0]
        ...
        for kind_ref, entity_id, revision in key.basis:
            resolve(tx, KnowledgeRef.model_validate({"entity_type": kind_ref, "id": entity_id, "revision": revision}))
        if key.intent_id is not None:
            ...
            basis = tuple((r["entity_type"], r["id"], r["revision"]) for r in strict_json_loads(intent["basis_json"]))
        ...
        tx.connection.execute(
            ...
            (*tx.owner, work_id, key.digest(), content, key.intent_id, key.intent_revision, priority),
        )
        return work_id
...
class DispatchRepository:
    def read(self, tx, *, operation_id):
        ...
        if (tx.purpose != "observe" or not tx.permissions.get("can_observe")
        ...
        if sha256(canonical_json_bytes(body)).hexdigest() != record["assignment_digest"]:
            raise DomainError("INPUT_DIGEST_CONFLICT", 409)
...
def _notification(tx, kind, payload):
    ...
    sequence = tx.connection.execute(
        "UPDATE vnext.task SET event_seq=event_seq+1 WHERE tenant_id=%s AND project_id=%s AND task_id=%s RETURNING event_seq", tx.owner,
    ).fetchone()[0]
    tx.connection.execute(
...
class Scheduler:
    def __init__(self, uow, *, ownership, accesses, snapshots, registry, control,
                 credential_issuer=None, policy=None):
        if not isinstance(uow, UnitOfWork) or not isinstance(ownership, SchedulerOwnership):
            raise ValueError("existing UnitOfWork and a real SchedulerOwnership are required")
        ...
        if len({(a.principal.tenant_id, a.principal.subject) for a in self.accesses}) != len(self.accesses):
    ...
    def _tasks(self):
        ...
        for access in self.accesses:
            if ("agent" in access.principal.roles or
                not access.principal.roles.intersection({"scheduler", "controller"})):
                raise DomainError("NOT_FOUND_OR_FORBIDDEN")
            with self.ownership.connection.transaction():
                self.ownership.assert_owned()
                for key, value in {
                    "tenant": access.principal.tenant_id, "subject": access.principal.subject,
                    "token_id": access.principal.token_id, "project": "", "task": "",
                    "clearance": "-1", "write": "false", "admit": "false", "request_purpose": "",
                }.items():
                    self.ownership.connection.execute("SELECT set_config(%s,%s,true)", ("wuji." + key, value))
                tasks = rows(self.ownership.connection.execute(
            ...
            for task in tasks:
                located.setdefault((access.principal.tenant_id, task["task_id"]), access)
        return located
    ...
    def _profile(self, tx, kind, config, receiver):
        try:
            ...
            body = profile["body"]
            disabled = {name: False for name in (
                "todo", "mode", "file_memory", "file_access", "skills", "shell", "web_search",
                "background_agents", "outer_loop", "auto_approval", "compaction", "restoration", "mcp",
            )}
            if (canonical_json_bytes(profile) != canonical_json_bytes(published)
                or profile["digest"] != sha256(canonical_json_bytes(body)).hexdigest()
                or profile["ref"] != body["ref"] or profile["revision"] != body["revision"]
                or body["work_kind"] != kind or body["lock_digest"] != config.runtime.lock_digest
                or body["capabilities"] != disabled
                or not body["instructions"] or not body["tool_definition_refs"]
                or any(type(body[n]) is not int or body[n] <= 0 for n in (
                    "max_context_records", "max_context_bytes", "max_output_tokens"))):
            ...
            for ref in refs:
                ...
                if (tool.approval_required or tool.allowed_target_kinds != ["workspace_read"]
                    or tool.name in names or ref not in executor.allowed_tool_refs
                    or executor.receiver_id != receiver["receiver_id"]
                    or executor.environment_ref != receiver["environment_ref"]):
                    raise ValueError("published tool/executor does not match current receiver")
                names.add(tool.name)
            return profile
    ...
    def _consume_events(self, tx):
        ...
        events = rows(tx.connection.execute(
            ...
            ORDER BY o.event_seq LIMIT 256""", tx.owner,
        ))
        for event in events:
    ...
    def _prepare(self, tx, now):
        ...
        intents = rows(tx.connection.execute(
            ...
            (i.tenant_id,i.project_id,i.task_id,i.entity_id,i.revision)) ORDER BY i.created_at,i.entity_id LIMIT 256""", tx.owner,
        ))
        if intents:
            profile = self._profile(tx, "explore", config, receiver)
            for intent in intents:
                key = WorkKey(
                    "intent:" + intent["entity_id"], intent["entity_id"], str(intent["revision"]),
                    profile["ref"] + ":" + profile["revision"], profile["digest"],
                    tuple((r["entity_type"], r["id"], r["revision"]) for r in strict_json_loads(intent["basis_json"])),
                    receiver["environment_ref"], intent["expected_output"],
                )
        ...
        if state.inflight_reason_work_id:
            ...
            if work and work["result_submission_id"]:
                with tx.connection.transaction():
                    try:
                        consumed = self.triggers.consume(tx, submission_id=work["result_submission_id"])
                    except DomainError as error:
                        if error.code not in {"STALE_EXECUTION", "reason_raw_reader_unavailable"}:
                            raise
            if work and not consumed and work["process_state"] == "exited":
                self.triggers.fail(tx, work_item_id=state.inflight_reason_work_id,
                                   reason_code="invalid_result" if work["result_submission_id"] else "missing_result", now=now)
        state = self.triggers.read(tx)
        if (state.pending_generation is None or state.inflight_reason_work_id or state.blocked_reason
            or state.retry_at is not None and state.retry_at > now):
            return
        existing_reason = tx.connection.execute(
            ...
            AND w.state NOT IN ('done','failed','cancelled') LIMIT 1""", tx.owner,
        ).fetchone()
        if not existing_reason:
            profile = self._profile(tx, "reason", config, receiver)
            key = WorkKey(
                f"reason:{state.pending_generation}:retry:{state.failure_count}", None, None,
                profile["ref"] + ":" + profile["revision"], profile["digest"], (),
                receiver["environment_ref"], "wuji.agent-payload.v2:reason_decision",
            )
    ...
    def _limits(self, tx, work, config, now):
        ...
        if (tx.task["activated_at"] is None
            or (now - tx.task["activated_at"]).total_seconds() >= limits.max_elapsed_seconds):
            raise DomainError("max_elapsed_seconds", 429)
        counts = row(tx.connection.execute(
            "SELECT * FROM vnext.admission_counter WHERE tenant_id=%s AND project_id=%s AND task_id=%s", tx.owner,
        ))
        if counts and (counts["model_attempts"] >= limits.max_model_requests
        ...
        if work["kind"] == "reason":
            reasons = tx.connection.execute(
                ...
                WHERE a.tenant_id=%s AND a.project_id=%s AND a.task_id=%s AND w.kind='reason'""", tx.owner,
            ).fetchone()[0]
    ...
    def _admit(self, tx, proposal, now):
        ...
        registration = row(tx.connection.execute(
        ...
        if key["profile_digest"] != profile["digest"] or key["environment_ref"] != receiver["environment_ref"]:
            raise DomainError("work_profile_changed", 409)
        old_block = row(tx.connection.execute(
        ...
        if not callable(getattr(issuer, "prepare", None)) or not callable(getattr(issuer, "bind_admitted_run", None)):
        ...
        if (manifest.tenant_id, manifest.project_id, manifest.task_id) != tx.owner:
            raise DomainError("INVALID_REFERENCE", 422)
        if work["kind"] == "reason":
            self.triggers.begin_reason(tx, work_item_id=work["work_item_id"], snapshot_id=manifest.snapshot_id)
        ...
        identity = RunIdentity.model_validate({
            ...
            "work_item_id": work["work_item_id"], "agent_run_id": run_id,
            "execution_epoch": str(tx.task["execution_epoch"]), "run_epoch": str(epoch),
            "runtime_attempt": str(tx.task["runtime_attempt"]), "receiver_id": receiver["receiver_id"],
        })
        assignment = WorkerAssignment.model_validate({
            "schema_version": "wuji.assignment.v2", "operation_id": operation_id,
            "identity": identity, "work_kind": work["kind"], "snapshot_id": manifest.snapshot_id,
            "profile_refs": [profile["ref"], config.model.ref, config.runtime.ref],
            "session_manifest_ref": None, "tool_definition_refs": profile["body"]["tool_definition_refs"],
            "limits": config.runtime.limits, "resume_reason": None,
        })
        tx.connection.execute(
            ...
            (*tx.owner, run_id, work["work_item_id"], receiver["receiver_id"], tx.task["execution_epoch"],
             epoch, tx.task["runtime_attempt"], receiver["environment_ref"], receiver["model_mode"], operation_id),
        )
        tx.connection.execute(
        ...
        definition = strict_json_loads(tx.task["definition_json"])
        authorization_expiry = datetime.fromisoformat(definition["task"]["authorization_expires_at"].replace("Z", "+00:00"))
        expires_at = min(authorization_expiry, tx.task["activated_at"] + timedelta(seconds=config.runtime.limits.max_elapsed_seconds))
        binding, credential_ref = issuer.prepare(
            tx, identity=identity, tool_definition_refs=tuple(profile["body"]["tool_definition_refs"]), expires_at=expires_at,
        )
        binding = RunCredentialBinding.model_validate(binding)
        if (binding.identity != identity or binding.subject != worker_subject(run_id)
            or binding.expires_at > expires_at or binding.expires_at <= now
            or set(binding.purposes) != {"model_request", "tool_request"}
            or tuple(binding.allowed_tool_refs) != tuple(profile["body"]["tool_definition_refs"])
            or not isinstance(credential_ref, str) or not credential_ref or len(credential_ref) > 256):
        ...
        event_seq = _notification(tx, "run.dispatch_requested", {
            "agent_run_id": run_id, "operation_id": operation_id, "assignment_digest": digest,
        })
        tx.connection.execute(
            ...
            (*tx.owner, run_id, work["work_item_id"], operation_id, manifest.snapshot_id,
             json_text(body), digest, credential_ref, event_seq),
        )
        tx.connection.execute(
        ...
        tx.connection.execute(
            "DELETE FROM vnext.scheduler_block WHERE tenant_id=%s AND project_id=%s AND task_id=%s AND work_item_id=%s", (*tx.owner, work["work_item_id"]),
        )
        return assignment
    ...
    def tick(self, *, now=None, limit=16):
                ...
                with self._transaction(access, task_id) as tx:
                    ...
                    try:
                        ...
                        tx.connection.execute(
                            ...
                            WHERE tenant_id=%s AND project_id=%s AND task_id=%s""", (error.code, *tx.owner),
                        )
                    else:
                        tx.connection.execute(
                            "UPDATE vnext.scheduler_state SET preparation_block_reason=NULL,preparation_release_condition=NULL WHERE tenant_id=%s AND project_id=%s AND task_id=%s", tx.owner,
                        )
                    items = rows(tx.connection.execute(
                        ...
                        ORDER BY s.ready_since,w.work_item_id""", tx.owner,
                    ))
                    if task_can_run(tx.task):
                        for w in items:
                            ...
                            eligible = dependencies_satisfied(tx, w["work_item_id"]) and intent_current(tx, w)
                            candidates.append(Candidate(tenant, task_id, w["work_item_id"], w["kind"], w["priority"], w["ready_since"], eligible))
            except DomainError as error:
        ...
        proposal = self.policy.select(SchedulingSnapshot(
            tuple(candidates), now, latest[1] if latest[0] else None,
            tuple(task_cursors.items()), limit,
        ))
        assignments = []
        for selected in proposal:
            ...
            try:
                with self._transaction(access, selected.task_id) as tx:
                    ...
                    tx.connection.execute(
                        "UPDATE vnext.scheduler_state SET last_selected=nextval('vnext.scheduler_selection_order') WHERE tenant_id=%s AND project_id=%s AND task_id=%s", tx.owner,
                    )
                    try:
                        with tx.connection.transaction():
                        ...
                        machine = error.code in {
                            "capacity_unavailable", "dispatch_guard_unsatisfied", "receiver_unavailable",
                            "worker_profile_unavailable", "snapshot_transaction_unavailable",
                            "credential_issuer_unavailable", "model_pool_unavailable", "CAPABILITY_UNAVAILABLE",
                            "credential_template_unavailable", "credential_encryption_unavailable",
                            "credential_key_unavailable", "credential_signing_unavailable",
                        }
                        block(tx, selected.work_item_id, error.code, machine=machine,
                              responsible="platform_operator" if error.status == 503 else "task_operator",
                              remedy="Recheck the actual registered prerequisite and current P05 dispatch guard" if machine else "Operator review required; cumulative limits are retained")

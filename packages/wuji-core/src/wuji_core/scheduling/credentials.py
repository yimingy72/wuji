...
class RunCredentialIssuer:
    def __init__(self, *, signing_key_resolver, encryption_key_resolver, public_key_resolver):
    ...
    def _template(self, tx, identity):
        template = row(tx.connection.execute(
    ...
    def _sign(self, template, claims):
        try:
            key = RSAKey.import_key(self._secret(self.signing_key_resolver, template["signing_key_ref"]))
            token = jwt.encode({"alg": "RS256", "kid": template["signing_kid"]}, claims, key)
            verifier = TokenVerifier(
                public_key_pem=self._secret(self.public_key_resolver, template["signing_key_ref"]),
                issuer=template["issuer"], audience=template["audience"],
            )
    ...
    def prepare(self, tx, *, identity, tool_definition_refs, expires_at):
        ...
        if expires_at.tzinfo is None or int(expires_at.timestamp()) <= int(now.timestamp()):
        ...
        binding = RunCredentialBinding(
            identity=identity, subject=worker_subject(identity.agent_run_id), token_id=str(uuid4()),
            expires_at=expires_at, purposes=["model_request", "tool_request"],
            session_lineage="run:" + identity.agent_run_id, allowed_tool_refs=list(tool_definition_refs),
        )
        claims = {
            "iss": template["issuer"], "aud": template["audience"],
            "sub": binding.subject, "tenant_id": identity.tenant_id,
            "roles": ["worker"], "jti": binding.token_id,
            "iat": int(now.timestamp()), "nbf": int(now.timestamp()), "exp": int(expires_at.timestamp()),
        }
        ...
        if (principal.subject != binding.subject or principal.token_id != binding.token_id
            or principal.tenant_id != identity.tenant_id or principal.roles != frozenset({"worker"})):
            raise DomainError("credential_binding_mismatch", 409)
        encryption_key = self._secret(self.encryption_key_resolver, template["encryption_key_ref"])
        ...
        aad = canonical_json_bytes({"credential_ref": credential_ref, "binding": document,
        ...
        tx.connection.execute(
            ...
            (*tx.owner, template["template_ref"], json_text(document), credential_ref, nonce, ciphertext),
        ).fetchone()
    ...
    def retrieve(self, tx, *, credential_ref, identity):
        ...
        value = row(tx.connection.execute(
            "SELECT * FROM vnext.read_scheduler_credential(%s,%s,%s,%s)", (*tx.owner, credential_ref),
        ))
        if not value:
            raise DomainError("NOT_FOUND_OR_FORBIDDEN")
        document = strict_json_loads(value["binding_json"])
        binding = RunCredentialBinding.model_validate(document)
        if binding.identity != identity or binding.expires_at <= datetime.now(timezone.utc):
            raise DomainError("STALE_EXECUTION", 409)
        aad = canonical_json_bytes({"credential_ref": credential_ref, "binding": document,
                                    "encryption_key_ref": value["encryption_key_ref"]})
        key = self._secret(self.encryption_key_resolver, value["encryption_key_ref"])
        return AESGCM(key).decrypt(bytes(value["nonce"]), bytes(value["ciphertext"]), aad).decode("utf-8")

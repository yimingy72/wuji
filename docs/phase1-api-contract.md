...
| POST | `P/tasks/{task_id}/commands` | 0.4实现 cancel；pause / resume 返回409 INVALID_TRANSITION |
...
0.4.0在上述权限上为Operator增加`task.read/task.create/task.control`，Viewer增加`task.read`。任务与事件按项目可读，回执还要求当前身份是原提交者；读取回执时仍须保有项目读取权限。任务的`allowed_actions`同时取决于当前权限和状态：本批仅Operator的queued任务可以取消。
...
0.4.0新生成的有效预览返回`can_create=true`和空blockers；受阻预览保留原因。迁移不修改旧B1预览的创建能力，旧预览必须重新生成。过期预览返回409 PREVIEW_EXPIRED，完整输入或摘要不符返回422，当前范围拒绝返回403 SCOPE_DENIED。
...
| 原键查询 404 | 可能尚未提交，不能证明原请求未执行 | 保留待确认状态；仅当前页面仍持有原请求时允许用户重送原键原请求 |
...
本批不启用回执自动删除。标签页sessionStorage只保存用户/项目、原键、命令种类、必要资源ID和时间；请求正文只保留在内存。刷新后仅按原键只读核对，不能猜测并重建原请求。注销或身份变化清理所属记录，项目切换中止旧请求并隔离迟到响应。
...
0.4的事件游标绑定用户、权限版本、项目、任务和位置，15分钟有效，页大小可调整。Task行的事件序号与任务状态同事务更新；快照读取同一行签发位置。当前可见详情每5秒串行同步，每轮最多补5页，隐藏暂停、终态停止周期轮询；连续3次失败后由用户手动重试。

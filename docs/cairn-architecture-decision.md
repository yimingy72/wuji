...
本批只更新文档，Phase1A保持partial，P0验收限定原实验，业务仍0.4.0。原共享检查预算已用426/600秒、剩174秒，真实4次额度已用完，不按重拆批次重置。文档仅diff/链接检查；后续最小场景为未启动不派发、同一Task Pod的agent容器内两个AgentRun共用kali容器、结果核对不重跑、工具越权拒绝、取消与迟到派发、旧queued不执行；不足则待测，不预先标集成通过。

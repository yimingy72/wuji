...
当前不能向用户宣称现场已可用。A8 只交付了无害 F1/F2 fixture、真实本地 BFF 验收 driver 和 87 项静态矩阵；没有固定集成 SHA 的浏览器/服务链实测，也没有访问指定现场实例。
...
- DeepSeek Key 不向 A8 请求、不写入浏览器/Worker/Git；DG2 需要 A0/A6 在受信网关内核对 SecretRef、usage 和费用。

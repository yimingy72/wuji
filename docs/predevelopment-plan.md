...
- 日期：2026-09-11；状态：用户已授权开始运行基础开发，完整0.5控制面尚未实施。
...
当前运行基础批次按其Spec执行有限依赖准备、定向离线单元检查和包构建；不运行浏览器、Kubernetes或模型。承接P0后本批新增82秒，当前累计508/600秒、剩92秒，具体以当前验收为准，真实4次模型额度已耗尽，不因拆批重置。预算不足如实待测，不把代码或文档交付冒充完整验收。

...
| 15 个 API 操作及业务 Schema | packages/contracts/openapi.yaml | 已可生成类型和校验；后端尚未实现 |
...
| 页面交互与主题 | docs/phase1-wireframes.md、DESIGN.md | 三栏风格与五套内置主题已确认；正式界面迁移、品牌标识和完整配置页面待对应阶段实现 |
...
已具备启动正式工程开发的架构、契约和交互基础。下一批工作进入工程初始化与 Phase 1 实现；首批开工任务包含尚未完成的后端、身份和数据决策，不以补齐所有远期页面或 Phase 2 的模型验证作为开工前提。当前尚无正式平台后端和 `apps/web` 实现。
...
| B：真实任务管理 | 项目权限、Scope 读取与服务端预览、幂等创建/取消请求、任务查询、调用账本与 Outbox | 浏览器创建的任务持久保存在数据库；刷新可查询；重复提交、版本冲突和无权限有实际集成检查 | A、P0-07 |

...
> 2026-09-10 当前入口：见 [背景索引](project-context.md) 和 [Phase 1C 前置草案](stages/phase-1c-prep/spec.md)。下文保留原 HTTP 纵向切片及依赖顺序；基础模型配置已前移到下一步规划，不能用“模型随后接入”排除配置管理与待规划的连接检查。Runtime 与 Harness 尚未实现。

...
## 2026-09-21 CORE-20260921-r1 当前入口

- 实际基线：`codex/vnext-maf@e06fee4816741d42f456192d2ea42b94fb485244`；5 组既有未跟踪证据目录保持用户所有，不读写、不暂存。
- 本轮改动：在现有控制、证据、Session、MAF 与完成服务上交付问题中心黑板、有状态问题级工作和信息驱动闭环；外围新增冻结。
- 当前第一项：C0 显式 v3/Profile 合同、公共字段与最小持久身份接线。最小检查为合同生成、旧 v2/新 v3/混版解析和新增迁移约束；通过即转 C1/C2，不运行全套历史矩阵。
- 后续顺序：C1 问题黑板与 C2 按需知识可并行落地，随后 C3→C4→C5；C6 只在固定构建上做一次适用的有限运行，机制、真实模型与现场结果分别记录。
...
| R04 视图/详情 snapshot 错配 | **已修复并完成 K8s/浏览器证据** | `20175ca` 的 v3 ViewEventBatch 携带新 `snapshot_id`，前端应用 patch 时同步更新快照身份；见 X04 证据 |
| R05 SSE cursor 未真正恢复基线 | **已修复并完成 K8s/浏览器证据** | `20175ca` 绑定 cursor 到 `view_revision+snapshot_id`；旧 cursor 得到 `VIEW_RESET_REQUIRED`，见 X04 证据 |
...
| X04（R04+R05） | 代码与限定 K8s/浏览器证据已完成；规模/跨 BFF 长连接仍待测 | `docs/vnext/evidence/P15/x04-live-stream-20260918/README.md`；7+4 stream tests、合同生成与 Web typecheck 通过 |
...
3. **X04（R04/R05）**：单 Task K8s SSE、stale-cursor reset 和浏览器无刷新更新已完成；仍需多标签公平性、跨 BFF 重启恢复与规模 p95。

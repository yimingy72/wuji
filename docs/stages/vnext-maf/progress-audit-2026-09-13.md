...
沿用已批准的 P/AC 和 M3–M5，只调整交付顺序。近期并行推进恢复、正式入口和本地 K8s 装配三条线；共享迁移、数据库和固定端口由单一集成人串行使用。并发按文件和依赖决定，不设人为数量上限。

用户在本次审核后再次明确本地 K8s 已可使用和部署测试，因此将部署验证提前为 1C，不等待工作台、数据治理或最终发布物完成。既有就绪记录已确认本地 `docker-desktop`、Ready 的 arm64 节点与 Pod 操作权限；当前新链路仍缺专用镜像/部署装配，`ops/vnext/` 目前只有迁移说明。这是应立即补齐的工程工作，不是环境不可用或缺授权。首次部署前只核对目标 context/namespace 和实际镜像，不重复完整环境审计。
...
| 1C：本地 K8s 部署测试 | SOL 与1A/1B并行准备 arm64 新核心/Worker/Supervisor 镜像、隔离测试 namespace、数据库/存储/测试身份与启动装配；沿用每 Task 一个 agent+kali 双容器 Pod。先部署已有可运行切片，再随功能接入验证，不等待最终 release.yaml | 实际新链路 Pod 可启动，Scheduler/Outbox 可派发至 Pod 内真实 child；合成模型与无害工具经实际网络接口返回结果，进程退出可核对 | 先验证一条启动→派发→工具→结果→退出路径及必要身份/授权边界，后续仅复测受变更影响的部署路径。固定镜像摘要、Pod UID 和代码绑定；不加载旧 Cairn/Pi 入口、不改旧部署，不重复宿主已过的完整矩阵 |
...
| 4：P16/P19 与部署完善 | 完成引用保留/GC/purge、受限衍生件与下载、审计/观测；旧数据只读副本归档；在1C实际部署基础上补齐发布清单和新增组件 | 可保留、可审计、可离线查历史的新运行包；复用此前实际 Task Pod 双容器证据并注明版本适用性 | 治理覆盖已发布引用与无引用对象，归档不产生可执行工作；只验证本批受影响的部署和出口约束，不把首次 K8s 运行推迟到这里 |
...
- 本文不新增测试成果或资产发现；本地 K8s 部署测试沿用用户已经提供并再次确认的授权。下次开发从 1A/1B/1C 继续，不重新进行全仓审计、不重开已关闭 M2、不恢复 Superpowers 流程。

...
3. **P11-C：正式创建与控制闭环。创建入口 + owner 启动命令已交付，worker start 门进行中。** `d0204db` 增加 `POST /api/v2/tasks`：0019 迁移的 `published_profile` 目录与 `vnext.create_task` SECURITY DEFINER 函数、`TaskService`、路由与 K8s 实测（201/幂等重放/409/匿名 401/浏览器入口不转发）；见[P11-C 创建入口证据](../../vnext/evidence/P11/task-creation-20260915/README.md)与[决定 D18](../../vnext/decision-register.md)。owner 侧 admission/容量/调度身份发布已由 `8d6c972` 交付（真实 PostgreSQL 2 项 + K8s 对已创建 Task 发布后 start/pause/cancel 202 与控制事件；该 Task `agent_run=0`，不作进程停止结论，见[证据](../../vnext/evidence/P11/task-admission-control-20260915/README.md)）。2026-09-15 追加：Pod 级停止观察已在真实 K8s 完成——`cancel` 受理后 5 秒内删除运行中的 Task Pod，Task 保持 reconciling 且不伪造退出，见[证据](../../vnext/evidence/P11/pod-stop-20260915/README.md)；同时完成全平面镜像按 `8d6c972` 重建滚动与过期凭据重签。**未关闭**：Pod 运行期间 runtime 持续 `runtime_dispatch_error`(DomainError)、worker 未启动（agent 无日志、run 停 registered），以及从部署配置自动推导 admission 行的 owner 工具；诊断已明确：receiver 在 Pod 停止后被禁用、runtime 固定 `execution_epoch=2` 而任务代次已推进（PermitDenied 正确拒绝）、租户尚无 `session_capability`；错误码与 pod-environment 状态日志已由 `56ba97d`/`b5cc4ca` 补齐。二次跟进确认：同代次原地重跑会被持久 inbox 以 `RECEIVER_IDENTITY_CONFLICT` 正确拒绝。下一步用**新 runtime attempt**（任务+runtime 配置同步渲染，并给每次尝试独立的 agent 状态卷或归档旧 inbox）验证 assignment 到达 supervisor，（资源命名已由 `905fd51` 修为按 attempt 分代：`wuji-task-…-a<N>-…`；完整 attempt 身份接线见[attempt-identity-wirings.txt](../../vnext/evidence/P11/pod-stop-20260915/raw/attempt-identity-wirings.txt)：任务资源、task-agent/task-kali Service 选择器、runtime pod_runtime、gates executors[].binding 必须一起移动），同时发布 tenant session capability，再补干净退出观测。
   2026-09-15 第四增量（owner 启动命令）：`d5a246a` 起交付 `ops/vnext/task_launch.py`，把「产品入口创建的 Task → definition 定稿 → admission/executor/Intent → `start` 202 → attempt 资源与 runtime/gates 绑定 → runtime 创建并注册 Pod → tenant session capability 绑定该 Pod UID」收敛为一条 owner 命令，真实 K8s 上四阶段全绿（Task `2abfdd57-…`，Pod uid 363d560a…，3 条 capability），随后 scheduler 接纳 reason 工作、runtime 投递 assignment、平台记录真实 started/exited 观测；证据见 [P11-C owner 启动](../../vnext/evidence/P11/task-roundtrip-20260915/README.md)，决定见 D19。**下一步（本项收尾）**：给 `WorkerBridge._await_start` 加有界拒绝码（哨兵为 STALE_EXECUTION/LIMIT_BLOCKED），据此修子进程 start 门被拒的因；定义零工具尝试 run 的结算语义（否则 work 项永久停在 reconciling/operations_unsettled）；决定受信 receiver 明确 `OPERATION_NOT_FOUND` 时的受控重投。

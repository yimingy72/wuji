...
2026-09-24取消收口切片：b18机制和外部四终态已通过，但Task仍`cancel/quiescing`且无CompletionEpoch。先在P05受权用户命令与Runtime受信故障入口写不同来源及仅新请求的持久待结算标记；P12增加内部取消提议，保留Goal precheck和原审查快照；现有`LaunchWorker.run_once`增一个有界、可重入结算lane，不增服务。close继续复用终态/操作结算守卫及报告冻结；未启动窄分支按Spec§3.3核对Launch步骤与执行事实。旧Task不自动扫描，b18只在新链通过后显式逐Task审计回填。最小验证仅三情形：新已停止且有成果取消关闭并冻结报告；确证未启动取消关闭；终态或操作未结算保持未关闭。固定代码和镜像来源，不重跑b18机制或收费模型。

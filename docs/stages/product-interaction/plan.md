...
修订说明（2026-09-10）：下述为已交付首版原型范围。用户现已要求以五类任务场景替代两条开发阶段入口，并补齐运行配置、平台出网与流量视图。新方案见 [场景与流量设计](../../scenario-execution-design.md)，状态 review-draft；用户同轮删除路径级访问限制。当前仅修订设计，未修改原型或业务实现。

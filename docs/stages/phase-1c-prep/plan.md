...
状态：draft / 待评审；本轮只规划。实施须先按应用 Plan 模式完成具体方案评审；此文件不表示原型或业务开发已经启动。
...
3. 场景及配置引用的来源：首个内置目录、真实能力状态、缺失模型/环境的 UI 表达；不预建全部配置中心。
4. 域名级 Scope 与历史路径 Scope 的并存/批准入口；新建授权职责与当前项目角色的映射。
5. 旧 queued 任务如何保留原版本/回执并阻止自动执行，新状态如何升级已发布校验器。增量迁移编号在实际主线复核后分配。

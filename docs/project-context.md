...
2026-09-15 首个真实浏览器读取路径已部署：独立 `api` Deployment 暴露P13受权读取，`wuji-web` 以Nginx+本地会话BFF双容器运行；浏览器通过HttpOnly会话读取同一真实Task的TopologySnapshot并在React Flow画布/列表显示。随后已接入同一snapshot的Intent/Origin记录详情和已保存历史快照选择，[P15记录详情](vnext/evidence/P15/record-detail-20260915/README.md)与[历史浏览器证据](vnext/evidence/P15/history-browser-20260915/README.md)保留精确请求和截图。完整工作台HTTP、Pod/镜像绑定和截图见[P14 K8s浏览器证据](vnext/evidence/P14/k8s-browser-20260915/README.md)，公共API切片见[P13 K8s API证据](vnext/evidence/P13/k8s-api-20260915/README.md)。这是本地只读机制身份；`8e0f30e` 随后将浏览器入口改为 Kubernetes `LoadBalancer` 直接管理的 `localhost:44180`，不再依赖宿主机 `kubectl port-forward`，见[P14入口证据](vnext/evidence/P14/k8s-loadbalancer-20260915/README.md)。[当前开发任务与下一项执行计划](stages/vnext-maf/next-development-plan-2026-09-15.md)已将下一项冻结为Layout CAS完整垂直切片；之后先收口P08 Session、P11正式创建/控制与P12可信完成，再实现ViewStream。生产OIDC仍独立待办。

2026-09-13 的[具体开发执行单](stages/vnext-maf/next-batch-plan.md)所列 A1/A2/B1/C0–C2 已产生对应局部结果并保留为历史计划截面；当前排序由[2026-09-15执行计划](stages/vnext-maf/next-development-plan-2026-09-15.md)接替。

...
export function GoalSummary({draft}:{draft:Draft}) {return <><h2>任务目标与完成条件</h2><p>{draft.content.objective||'尚未填写任务目标'}</p><h3>完成条件</h3>{draft.completionCriteria.trim()?<ul>{draft.completionCriteria.split('\n').filter(x=>x.trim()).map((x,i)=><li key={i}>{x}</li>)}</ul>:<p className={s.muted}>尚未填写</p>}{draft.supplementalHints&&<><h3>补充线索</h3><p className={s.preservedText}>{draft.supplementalHints}</p></>}</>;}
export function DraftSummary({draft,model}:{draft:Draft;model?:Profile}) {return <><div className={s.summaryTitle}>任务摘要</div><h2 className={s.summaryName}>{draft.content.name||'未命名任务'}</h2><p className={s.muted}>{draft.content.objective||'尚未填写任务目标'}</p><ScopeSummary draft={draft} compact/>

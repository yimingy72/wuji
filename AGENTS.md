...
- 用户最新明确：测试和开发全部交给子代理；主代理只负责架构、分工和质量审查。当前Core CTF后端子代理统筹Launch修复、本地集成提交、独立Core镜像/环境更新及机制验收；前端子代理负责F4浏览器验证。历史记录按当时实际执行者保留，不追溯改写。新子代理使用`gpt-6-sol`、`reasoning_effort=high`。
...
本次模型分工按用户2026-09-19最新决定：后续子代理按任务难度仅使用 `gpt-5.6-luna/xhigh` 或 `gpt-5.6-sol/high`、`gpt-5.6-sol/xhigh`；禁止使用 GPT-6 系列和 GPT-5.6 Terra。覆盖旧 `gpt-6-astra/low`、核心开发可用GPT-6及统一Sol/xhigh等旧限定；主代理保留当前模型并负责架构判断、集成及最终质量。已有测试证据按实际执行者记录，不因模型交接重跑。读取 v2 执行台账，不继承 v1 未完成准备记录的状态。

2026-09-13 用户最新决定：停止遵循 Superpowers 流程，按已批准 Spec 直接推进功能闭环，减少不必要 token。保留必要验证、原始证据和具体风险审查；不再强制开工 RED 仪式、反复 SOURCE_ONLY 复审、交接模板、复杂报告或状态轮询。旧台账只作历史参考，常规已授权开发不等待流程批准。
...
- 平台按五类场景提供默认 Goal/完成条件模板，创建者可以自定义；任务固定模板来源及实际目标文本。模板、自定义目标与 Agent 摘要不能扩大授权、解除禁止项或代替实际执行/覆盖证据。具体模板和阶段输出合同按对应 Spec 冻结，当前已确认 Reason 读取已有证据，主动补证通过普通 Intent 交给 Explore；不自行扩大为 Reason 内主动目标工具。
